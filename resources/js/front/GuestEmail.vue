    ...
    <div class="mt-4">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Guest Email List</h3>

              <div class="box-tools">
                  <button class="btn btn-success" @click="newModal">Add Email<i class="fa fa-envelope fa-fw"></i></button>
              </div>
            </div>
            <!-- /.box-header -->
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            ...
            <input v-model="form.email" type="text" class="form-control" :class="{ 'is-invalid': 
...
<script>
    export default {
        ...
        methods: {
		getResults(page = 1) {
			axios.get('api/guestemail?page=' + page)
				.then(response => {
            ...
            updateUser(){
                this.$Progress.start();
                this.form.put('api/guestemail/'+this.form.id)
                .then(()=>{
                    $('#addNew').modal('hide');
                            swal(
                            'Updated!',
            ...
            deleteUser(id){
                ...
                confirmButtonText: 'Yes, delete it!'
                    ...
                    if(result.value){
                        this.form.delete('api/guestemail/'+id).then(()=>{
                            swal(
                            'Deleted!',
            ...
            loadUsers(){
                    axios.get("api/guestemail").then(({ data }) => (this.users = data));
            },
          createUser(){
              this.$Progress.start();
              this.form.post('api/guestemail')
              .then(()=>{
                    $('#addNew').modal('hide');
                            swal(
                            'Updated!',
